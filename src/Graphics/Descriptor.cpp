...
namespace MilkShake
{
    namespace Graphics
    {
        void Descriptor::SetBindings(const VkDevice device, std::vector<VkDescriptorSetLayoutBinding> _bt)
        {
            uint32_t maxSets = 1;  // 1 is good enough for us.  In general, may want more;
            bindingTable = _bt;
        ...
        void Descriptor::Write(VkDevice& device, uint32_t index, const VkBuffer& buffer)
        {
        ...
        void Descriptor::Write(VkDevice& device, uint32_t index, const VkDescriptorImageInfo& textureDesc)
        {
        ...
        void Descriptor::Write(VkDevice& device, uint32_t index, const std::vector<VkDescriptorImageInfo>& textures)
        {
            VkWriteDescriptorSet writeSet{ VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET };
            writeSet.dstSet = descSet;
            writeSet.dstBinding = index;
            writeSet.dstArrayElement = 0;
            writeSet.descriptorCount = textures.size();
            writeSet.descriptorType = bindingTable[index].descriptorType;
            writeSet.pImageInfo = textures.data();
        ...
        void Descriptor::Write(VkDevice& device, uint32_t index, const VkAccelerationStructureKHR& tlas)
        {
